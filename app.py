...
with left_col:
    ...
    K = st.slider("K: Mean Reversion Rate", min_value=0.0, max_value=3.0, value=0.0, step=0.01)
    ...
    st.write(2*K*theta)
    sigma = st.slider("sigma: Volatility", min_value=0, max_value=sqrt(np.sqrt(2*K*theta)), value=float(sigma_cal), step=0.01)
    st.write(sigma**2)
